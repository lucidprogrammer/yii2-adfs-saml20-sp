 ...
 *
 */
extract(parse_url(getenv("SAML_STORE_DATABASE_URL")));
$dbname= substr($path, 1);
$dbtype=getenv('DBTYPE') ? getenv('DBTYPE') : 'pgsql';
// we have now $user $pass $host
$config = array(
     ...
     * reverse proxy).
     */
    'baseurlpath' => $_SERVER['SERVER_NAME'] === 'localhost' ? 'saml/' : 'https://'. $_SERVER['SERVER_NAME']. '/saml/',
    'certdir' => '/var/www/html/cert/',
    'loggingdir' => 'log/',
    'datadir' => 'data/',
     ...
     */
    'auth.adminpassword' => getenv('adminpassword'),
     ...
     */
    'technicalcontact_name' => getenv('technicalcontact_name'),
    'technicalcontact_email' => getenv('technicalcontact_email'),
     ...
     * syntaxes.
     */
    //'mysql:host=mysql;dbname=samldb',

    // 'store.sql.dsn' => $dbtype.':host='.$host. ';dbname='.$dbname,
     ...
     */
    // 'store.sql.prefix' => 'simpleSAMLphpsp',
